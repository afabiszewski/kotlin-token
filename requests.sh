...
curl -XGET -H"Authorization: ${JWT}" localhost:8080/groups/admin

printf "\n${RED}GET getGroup -> 404${NC} -> "
curl -XGET -H"Authorization: ${JWT}" localhost:8080/groups/groupDoesNotExist
...
curl -XPOST -H"Authorization: ${JWT}" -H"Content-Type: application/json" -d"{\"name\":\"curlGroup_$group\"}" localhost:8080/groups

printf "\n${RED}POST createGroup -> 403${NC} -> "
curl -XPOST -H"Authorization: ${JWT}" -H"Content-Type: application/json" -d"{\"name\":\"curlGroup_$group\"}" localhost:8080/groups

printf "\n${RED}GET deleteGroup -> 404${NC} -> "
curl -XDELETE -H"Authorization: ${JWT}" localhost:8080/groups/groupDoesNotExist

printf "\n${GREEN}GET deleteGroup -> 200${NC} -> "
curl -XDELETE -H"Authorization: ${JWT}" localhost:8080/groups/curlGroup_"$group"
